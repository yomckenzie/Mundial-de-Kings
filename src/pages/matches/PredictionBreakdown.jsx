...
// ─────────────────────────────────────────────────────────────────────
// Sub-componentes del breakdown post-evaluación (Task 6)
// ─────────────────────────────────────────────────────────────────────
...
// Para predicciones nuevas (3-pasos) muestra los 3 picks.
// Para predicciones legacy (pred_team1/pred_team2) muestra marcador simple.
function renderPickSummary(existing, match) {
  ...
  const methodLabel = existing.pred_method === '90' ? '90 min'
    ...
    : '?';
  const penaltyStr = existing.pred_method === 'pen'
    && existing.pred_penalty_team1 != null
    && existing.pred_penalty_team2 != null
      ? ` · ${existing.pred_penalty_team1}-${existing.pred_penalty_team2}`
      : '';
  return `${winnerLabel} · ${methodLabel}${penaltyStr}`;
}
...
export function ExistingPredictionPanel({ existing, match, isAdmin, resultKnown, predHit }) {
    ...
    return (
      <div className="space-y-1">
        ...
        <div className="text-center text-[11px] text-amber-600 dark:text-amber-400 font-medium py-1.5 px-2 rounded-lg bg-amber-50 dark:bg-amber-950/20 space-y-0.5">
          ...
          <p className="font-bold">hasta 150 pts</p>
        </div>
      </div>
    );
  }
  ...
  // Predicción nueva (3-pasos). Mostrar desglose por componente + total pts.
  // notApplicable=true para "Penales" cuando result_method≠pen o el usuario
  // NO apostó a penales (la fila sale con "⏸ no aplica").
  const penaltyNotApplicable = match.result_method !== 'pen' || existing.pred_method !== 'pen';
  return (
    <div className="space-y-1.5">
      ...
      <div className="space-y-1">
        ...
        <PtsRow label="Penales" correct={existing.penalty_correct} pts={50} notApplicable={penaltyNotApplicable} />
